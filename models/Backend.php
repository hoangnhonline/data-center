...
define('DB_USER', 'nghien_center');
define('DB_PASS', 'centernghienbiz');
define('DB_NAME', 'nghien_center');
define('PAGE_SHOW',5);
define('LIMIT',20);
...
class Backend {
    ...
    function __construct() {
        ...
        mysql_connect(DB_HOST, DB_USER, DB_PASS) or die("Can't connect to server");
    ...
    function firstSetting($site_id, $site_url, $dataSetting){
       
        $string_data = $this->getListImport($site_id, 32);
       
        $cateArr = $this->getList('cate_setting');

        $url = $site_url.'/import-data.php';
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, 
            array(
                'post' => $string_data,
                'cate' => json_encode($cateArr['data']),
                'setting' => json_encode($dataSetting)
            )
        );
    ...
    function importPost($site_id, $site_url){
       
        $string_data = $this->getListImport($site_id, 8);
    ...
    function updateSetting($site_url, $dataSetting){       
    ...
    function getListImport($site_id, $limit=8){
        $dataArr = array();
        $sql = "SELECT * FROM post WHERE status = 1 ORDER BY RAND() LIMIT 0,$limit";
        ...
        while($row = mysql_fetch_assoc($rs)){
            ...
            $dataInput['id'] = $row['id'];
            $dataInput['status'] = 0;
            $this->update('post', $dataInput);

            $data2['post_id'] = $row['id'];
            $data2['site_id'] = $site_id;
            $this->insert('site_video', $data2);
        }
    ...
    function getListVideo($status = -1, $offset, $limit) {        
        try{           

            $arrReturn = array();
            $sql = "SELECT * FROM post WHERE (status= $status OR $status = -1) ";                     
